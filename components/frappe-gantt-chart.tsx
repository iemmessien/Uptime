...
export function UptimeGanttChart({ type, selectedDate }: UptimeGanttChartProps) {
   ...
   const renderGanttChart = () => {
      ...
      // Sort uptime data according to POWER_SUPPLIES order, then by start time
      const sortedUptimeData = [...uptimeData].sort((a, b) => {
         ...
         // If same power supply, sort by start time
         return new Date(a.startTime).getTime() - new Date(b.startTime).getTime()
      })

      // Create tasks - each uptime record is a separate bar
      const tasks = sortedUptimeData.map((uptime) => {
         ...
         return {
            id: `task-${(uptime as any).id || uptime.powerSupply}-${start.getTime()}`,
